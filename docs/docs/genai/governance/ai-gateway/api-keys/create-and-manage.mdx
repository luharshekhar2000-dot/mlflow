...
description: Centrally store and manage LLM provider API keys in MLflow AI Gateway. Create reusable, encrypted LLM connections to share securely across multiple endpoints.
---
...
5. Enter your API key (displayed as a masked input for security)
...
The API key values remain masked for security.

### Editing Connections

To update the API key for a provider:
